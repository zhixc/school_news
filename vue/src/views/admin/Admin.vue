<template>
  <div style="padding: 10px">
    ...
    <div>
      ...
      <el-button  v-if="roleId === 1" type="primary" @click="add" icon="el-icon-plus" style="margin: 10px">新增</el-button>
    </div>
    ...
    <el-table
      ...
      <el-table-column
        ...
        <template #default="scope">
          ...
          <span v-if="scope.row.roleId === 2">编辑人员</span>
          <span v-if="scope.row.roleId === 3">普通用户</span>
        </template>
      </el-table-column>
      ...
      <el-table-column
        ...
        <template #default="scope">
          ...
          <el-popconfirm title="确定删除？" @confirm="handleDelete(scope.row.userId)">
            <template #reference>
      ...
      <el-dialog title="提示" :visible.sync="dialogVisible" width="80%">
        <el-form :model="form" label-width="120px">
          ...
          <el-form-item label="角色">
            <el-radio v-model="form.roleId" label="1">管理员</el-radio>
            <el-radio v-model="form.roleId" label="2">编辑人员</el-radio>
            <el-radio v-model="form.roleId" label="3">普通用户</el-radio>
          </el-form-item>
        </el-form>
        <template #footer>
...
<script>
  export default {
    ...
    methods: {
      load() { // 加载表格数据
        // axios 发起 get 请求调用接口 http://localhost:8081/user/list
      ...
      save() { // 新增对话框里面的 确认按钮 保存方法
        if (this.form.userId) { // 判断新增（或编辑）对话框中的 form 的 id 是否存在
          // 存在，说明执行的是编辑修改操作
          // axios发起 put 请求 调用接口 http://localhost:8081/user/update
          // 传入参数 form
          this.$axios.put("/api/user/update", this.form).then(res => {
          ...
          // axios发起 post 请求 调用接口 http://localhost:8081/user/save
          // 传入参数 form
          this.$axios.post("/api/user/save", this.form).then(res => {
